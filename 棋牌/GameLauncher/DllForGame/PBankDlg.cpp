...
void CPBankDlg::OnBnClickedAccountOK()
{
	...
	//CString strFormat = fMsg.GetKeyVal("PBankDlg", "TransferConfirm", "您将要给ID号为[%d]的玩家转帐%s金币，是否确定转帐？");
    ...
    {
		BZSoundPlay(this, "..\\music\\错误提示.mp3", 0);
		strFormat = fMsg.GetKeyVal("PBankDlg", "TransferConfirmId", "您将要给ID号为[%d]的玩家转帐%s金币，是否确定转帐？");
...
// lxl 2010-11-17 设置获取的昵称
void CPBankDlg::SetNickName(TCHAR* szNickName)
{
	//OutputDebugString("设置昵称lxl");
...
void CPBankDlg::OnIDKillFocus()
{
	...
	//OutputDebugString("游戏中发送获取昵称 lxl");
    // 发送获取用户昵称消息
...
void CPBankDlg::ChangeDigitToChinese(__int64 i64Num, CString &strResult)
{
	...
	for(i = size-1; i >= 0; i--)
	{
		...
		{
			if(i-1 == -1) break; 
			if(digitArray[i-1] != 0 && (strcmp(szUnit[i],"万")!=0 && strcmp(szUnit[i],"亿")!=0)) //非亿、万位为0时处理，不连续时加入零。
				strResult += szDigit[digitArray[i]];
		}
		else 
			strResult += szDigit[digitArray[i]];		
	
		//加入单位 //zht2011-11-29 修改字符串比较
		if(digitArray[i] == 0 && (strcmp(szUnit[i],"万")!=0 && strcmp(szUnit[i],"亿")!=0) ) //非亿、万位为0时跳过
			continue; 
