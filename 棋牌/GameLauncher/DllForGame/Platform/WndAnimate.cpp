...
bool CWndAnimate::Create(CWnd *pParent)
{
	...
	if (CreateEx(0, AfxRegisterWndClass(0/*,AfxGetApp()->LoadStandardCursor(IDC_HAND)*/), NULL, WS_POPUP,
...
///@param[in] iX, iY 座标
///@return void
void CWndAnimate::BeginAnimate(int iX, int iY, int iWidth, int iHeight, int nRound, int iFlip)
{
	if (GetIsPlaying())
	{
		return;
	...
	m_bPlaying = true;
	SetTimer(TIME_FRAME, m_iFlip, NULL);
