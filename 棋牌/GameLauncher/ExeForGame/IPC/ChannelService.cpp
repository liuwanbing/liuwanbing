...
CChannelService::CChannelService()
{
	...
	//设置接口
	m_IPCRecvCopyData.SetChannelMessageSink(this);
...
BOOL CChannelService::OnCopyData(CWnd * pWnd, COPYDATASTRUCT * pCopyDataStruct)
{
	...
	if ((hWndSend==m_hWndServer)&&m_IPCRecvCopyData.OnCopyData(hWndSend,pCopyDataStruct)) 
		return TRUE;
	return __super::OnCopyData(pWnd, pCopyDataStruct);
}

//设置事件
bool __cdecl CChannelService::SetChannelEventSink(IChannelEventSink * pIUnknownEx)
{
	if (pIUnknownEx!=NULL)
	{
		m_pIChannelEventSink=(IChannelEventSink *)pIUnknownEx;
...
//设置消息
bool __cdecl CChannelService::SetChannelMessageSink(IChannelMessageSink * pIUnknownEx)
{
	if (pIUnknownEx!=NULL)
	{
		m_pIChannelMessageSink=(IChannelMessageSink *)pIUnknownEx;
...
bool __cdecl CChannelService::CreateChannel(HWND hWndServer)
{
	...
	//发送连接通知
	return SendData(IPC_MAIN_IPC_KERNEL,IPC_SUB_IPC_CLIENT_CONNECT);
}
...
bool __cdecl CChannelService::CloseChannel(bool bNotifyServer, bool bNotifySink)
{
	...
	//发送关闭消息
	if (bNotifyServer==true) SendData(IPC_MAIN_IPC_KERNEL,IPC_SUB_IPC_CLIENT_CLOSE);
...
bool __cdecl CChannelService::OnChannelMessage(const IPC_Head * pHead, const void * pIPCBuffer, WORD wDataSize, HWND hWndSend)
{
	...
	//OutputDebugString("[aa]SDK  __cdecl CChannelService::OnChannelMessage 00");
	return true;
}
...
LRESULT CChannelService::OnIPCMessage(WPARAM wParam, LPARAM lParam)
{
	...
	try
	{
		if (IPCPacket.Head.wMainCmdID==IPC_MAIN_IPC_KERNEL)	
		{
			//内核消息
			...
			case IPC_SUB_IPC_CLIENT_CONNECT:	//连接消息
				{
					...
					SendCopyData.SendData(IPC_MAIN_IPC_KERNEL,IPC_SUB_IPC_SERVER_ACCEPT);
		...
		else	//外围消息
		{
			if (m_pIChannelMessageSink!=NULL)
			{
				...
				//OutputDebugString("[aa]SDK  CChannelService::OnIPCMessage");
			}
		}
	}
	catch (...)	{ }
