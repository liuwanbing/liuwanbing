...
void CNormalBitmapButtonEx::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	else if (m_ButtonBitmap.IsNull()==false)//以下为兼容以前的操作
	{
		...
		else if (m_bIsMouseMove==true) 
			iDrawPos=iPartWidth*1;///////鼠标经过了
	//	else if (lpDrawItemStruct->itemState&ODS_FOCUS) 
	//		iDrawPos=iPartWidth*2;///鼠标激活了，按下了
		else 
			iDrawPos=0;
		if (m_bExpand==false) 
			ImageHandle.BitBlt(pDC->GetSafeHdc(),ClientRect,CPoint(iDrawPos,0));
		else 
			ImageHandle.StretchBlt(pDC->GetSafeHdc(),ClientRect,CRect(iDrawPos,0,iDrawPos+iPartWidth,ImageHandle.GetHeight()));
	}
...
void CNormalBitmapButtonNoState::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	//定义变量
	CRect ClientRect;
	GetClientRect(&ClientRect);
	...
	{
		CGameImageHelper ImageHandle(&m_ButtonBitmap);
		int iPartWidth=ImageHandle.GetWidth(),iDrawPos=0;
		if (lpDrawItemStruct->itemState&ODS_DISABLED) //失效了
		{
			bDisable=true;
			iDrawPos=0;
	...
	else if (CGameImageLink::m_ButtonResource.m_BackPic.IsNull()==false)
	{
		...
		else if (m_bIsMouseMove==true) iDrawPos=0;//////鼠标经过了
		else if (lpDrawItemStruct->itemState&ODS_FOCUS) iDrawPos=0;///鼠标激活了，按下了
		else iDrawPos=0;
		ImageHandle.StretchBlt(pDC->GetSafeHdc(),ClientRect,CRect(iDrawPos,0,iPartWidth+iDrawPos,iHeight));
	}
