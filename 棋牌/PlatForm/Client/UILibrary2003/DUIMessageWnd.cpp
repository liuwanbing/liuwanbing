...
{
	if (wParam == IDT_TIME_DEC)
	{
		//定时时间到，关闭窗口
		if (--m_nLeftTime <= 0)
		{
			::KillTimer(m_hWnd,IDT_TIME_DEC);
			m_uRetFlag = IDOK;
			Close();
			return 0;
		}
		...
		{
			char szBuf[32] = {0};
			_stprintf(szBuf,"确定(%d)",m_nLeftTime);
			//2013-01-30 wangzhitu
			CString strT("");
			strT.Format("wzd DSQ Num=%d",m_nLeftTime);
			OutputDebugString(strT);
			m_pOKBtn->SetText(szBuf);
		}		
...
void CMessageWndDu::Init() 
{
	...
	CControlUI* pOKCtn = m_pm.FindControl("Ctn_OK");

	if (NULL != pOKCtn)
	{
		int idx = 0;
		CStdPtrArray* pBtnCtrls = m_pm.FindSubControlsByClass(pOKCtn,_TEXT("ButtonUI"));
		CButtonUI* pBtnCtrl = NULL;
		do 
		{
			pBtnCtrl = static_cast<CButtonUI*>(pBtnCtrls->GetAt(idx++));

			if (NULL != pBtnCtrl)
			{
				if (_tcscmp(pBtnCtrl->GetName(),_T("Btn_OK")) == 0)
				{
					m_pOKBtn = pBtnCtrl;
					//按钮能够动态显示
					m_pOKBtn->SetAttribute("align","center");
					break;
				}
			}
...
//当服务器未启动时，提示“游戏升级中，敬请期待"
void CMessageWndDu::SetBKImage()
{
	m_pTextTemp->SetVisible(false);
	CControlUI* pCtrl = static_cast<CControlUI*>(m_pm.FindControl("Img_logo"));
	if (pCtrl)
	{
		pCtrl->SetVisible(false);
	}

	pCtrl = static_cast<CControlUI*>(m_pm.FindControl("Lab_Main"));
	if (pCtrl)
	{
		pCtrl->SetVisible(false);
	}
	
	pCtrl = static_cast<CControlUI*>(m_pm.FindControl("Bnt_Close"));
	if (pCtrl)
	{
		pCtrl->SetVisible(false);
	}
	CContainerUI *pUI = static_cast<CContainerUI*>(m_pm.FindControl(_T("ContainerUI1")));
	if (pUI)
	{
		pUI->SetBkImage("dialog\\NoStart.png");
	}
}
...
//设置倒计时
void CMessageWndDu::SetLeftTime(int nLeftSecond)
{
	m_nLeftTime = nLeftSecond;

	if (m_nLeftTime != -1)
	{
		::SetTimer(m_hWnd,IDT_TIME_DEC,1000,NULL);
		...
		if (NULL != m_pOKBtn)
		{
			...
			//2013-01-30 wangzhitu
			CString strT("");
			strT.Format("wzd DSQ Begin NO.=%d",m_nLeftTime);
			OutputDebugString(strT);
			m_pOKBtn->SetText(szBuf);
		}
	}
