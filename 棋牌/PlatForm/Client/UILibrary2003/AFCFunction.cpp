...
LONG WINAPI ExceptionFilter(PEXCEPTION_POINTERS pExceptionInfo)
{
	...
	//AFCMessageBox(szError,"AMG系统");
	 DUIMessageBox(NULL,MB_ICONINFORMATION|MB_OK,"AMG系统提示",szError);
...
EXT_CLASS UINT DUIMessageBox(HWND hParentWnd,UINT style,LPCTSTR lpTitle,LPCTSTR lpMainText,...)
{
	...
	if (!strcmp(szMessage, _T("服务器没有启动")))
	{//特殊处理下，当游戏服务器未启动时，更改此提示，并重新设置下背景图片
		wndMsg.SetBKImage();
	}
	wndMsg.ShowModal();
...
//只有一个OK按钮，而且固定标题
EXT_CLASS UINT DUITimerMsgBox(HWND hParentWnd,UINT uSecond,LPCTSTR lpMainText,...)
{
	// 考虑到在xml编辑器中使用<>符号不方便，可以使用{}符号代替
	// 支持标签嵌套（如<l><b>text</b></l>），但是交叉嵌套是应该避免的（如<l><b>text</l></b>）
	// The string formatter supports a kind of "mini-html" that consists of various short tags:
	...
	wndMsg.CenterWindow();
	wndMsg.ShowWindow(SW_SHOW);
	wndMsg.SetTitle(_T("提示信息"));
	wndMsg.SetMainText(szMessage,true);
	wndMsg.SetStyle(MB_OK|MB_ICONINFORMATION);
	wndMsg.SetLeftTime(uSecond);
	wndMsg.ShowModal();
