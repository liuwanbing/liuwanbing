...
UINT CGameImage::LockResource()
{
	if (m_uLock==0)
	{
		...
		else
		{
			...
			TCHAR * pchar=m_strFileName.GetBuffer(m_strFileName.GetLength()+1);
...
//增加引用
UINT CGameImageEx::LockResource()
{
	if (m_uLock==0)
	{
		if (m_hResourceDLL!=NULL)
		{
			m_Image.LoadFromResource(m_hResourceDLL,m_uResourceID);
		}
		else
		{
#ifndef BZ_ZLIB
			m_Image.Load(m_strFileName);
			m_ImagePlus.LoadFromFile(CA2W(m_strFileName));
#else
			CUnZipRes unZipRes;
			IStream * pStream=NULL;
			TCHAR * pchar=m_strFileName.GetBuffer(m_strFileName.GetLength()+1);
			pStream=unZipRes.LoadFileResFromZip(pchar);
			m_Image.Load(pStream);
			m_ImagePlus.LoadFromStream(pStream);
			if(pStream!=NULL)
			{
				pStream->Release();
				pStream=NULL;
...
//减少引用
UINT CGameImageEx::FreeResource()
{
	if (m_uLock>0) m_uLock--;
	if (m_uLock==0)
	{
...
///绘图函数
BOOL CGameImageEx::BitBlt(HDC hDestDC, int xDest, int yDest, DWORD dwROP)
{
	if(m_Image.IsNull())
		return FALSE;
	else
	{
		return m_ImagePlus.DrawImage(CDC::FromHandle(hDestDC),xDest,yDest);
	}
}

///绘图函数
BOOL CGameImageEx::BitBlt(HDC hDestDC, const POINT & pointDest, DWORD dwROP)
{
	if(m_Image.IsNull())
		return FALSE;
	else
	{
		return m_ImagePlus.DrawImage(CDC::FromHandle(hDestDC),pointDest.x,pointDest.y);
	}
}

///绘图函数
BOOL CGameImageEx::BitBlt(HDC hDestDC, int xDest, int yDest, int nDestWidth, int nDestHeight, int xSrc, int ySrc, DWORD dwROP)
{
	if(m_Image.IsNull())
		return FALSE;
	else
	{
		return m_ImagePlus.DrawImage(CDC::FromHandle(hDestDC),xDest,yDest,nDestWidth,nDestHeight,xSrc,ySrc,nDestWidth,nDestHeight);
	}
}

///绘图函数
BOOL CGameImageEx::BitBlt(HDC hDestDC,const RECT & rectDest, const POINT & pointSrc, DWORD dwROP)
{
	if(m_Image.IsNull())
		return FALSE;
	else
		return m_ImagePlus.DrawImage(CDC::FromHandle(hDestDC),rectDest.left,rectDest.top,rectDest.right - rectDest.left,rectDest.bottom - rectDest.top,pointSrc.x,pointSrc.y,rectDest.right - rectDest.left,rectDest.bottom - rectDest.top);
}

///绘图函数
BOOL CGameImageEx::StretchBlt(HDC hDestDC, int xDest, int yDest, int nDestWidth, int nDestHeight, DWORD dwROP)
{
	if(m_Image.IsNull())
		return FALSE;
	else
		return m_ImagePlus.DrawImage(CDC::FromHandle(hDestDC),xDest,yDest,nDestWidth,nDestHeight,0,0,m_ImagePlus.GetWidth(),m_ImagePlus.GetHeight());
}

///绘图函数
BOOL CGameImageEx::StretchBlt(HDC hDestDC, const RECT& rectDest, DWORD dwROP)
{
	if(m_Image.IsNull())
		return FALSE;
	else
		return m_ImagePlus.DrawImage(CDC::FromHandle(hDestDC),rectDest.left,rectDest.top,rectDest.right,rectDest.bottom,0,0,m_ImagePlus.GetWidth(),m_ImagePlus.GetHeight(),dwROP);
}

///绘图函数
BOOL CGameImageEx::StretchBlt(HDC hDestDC, int xDest, int yDest, int nDestWidth, int nDestHeight, int xSrc, int ySrc, int nSrcWidth, int nSrcHeight, DWORD dwROP)
{
	if(m_Image.IsNull())
		return FALSE;
	else
		return m_ImagePlus.DrawImage(CDC::FromHandle(hDestDC),xDest,yDest,nDestWidth,nDestHeight,xSrc,ySrc,nSrcWidth,nSrcHeight);
}

///绘图函数
BOOL CGameImageEx::StretchBlt(HDC hDestDC, const RECT& rectDest, const RECT& rectSrc, DWORD dwROP)
{
	if(m_Image.IsNull())
		return FALSE;
	else
		return m_ImagePlus.DrawImage(CDC::FromHandle(hDestDC),rectDest.left,rectDest.top,rectDest.right,rectDest.bottom,rectSrc.left,rectSrc.top,rectSrc.right,rectSrc.bottom);
}
