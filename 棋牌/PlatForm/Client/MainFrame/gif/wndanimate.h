...
public:
	...
	///@param[in] iFlip 每帧间隔
	///@return void
	void BeginAnimate(int iX, int iY, int iWidth, int iHeight, int nRound = 1, int iFlip = 100,bool IsLoop = false);

	void AppendGif(const wchar_t *pszFileName);
...
private:
	...
	int							m_iFlip[200];						///< 帧间间隔最多200帧	
	int							m_nFrameCount;					///< 共有几帧
	int							m_nTime;						///< 播放几次
	int							m_nTotalRound;					///< 总共播放多少帧
	bool                        m_IsLoop;                       ///是否循环
