...
void CGameUserManage::FindOnLineUser(BYTE bDeskIndex, TUserList& uselist)
{
	uselist.clear();
	INT_PTR uActiveCount=m_ActiveUserPtrArray.GetCount();
	for (int i=0;i<uActiveCount;i++)
	{
		...
		if (pFindUserItem!=NULL)
		{
			if (pFindUserItem->GameUserInfo.bDeskNO==bDeskIndex)
			{
				if (pFindUserItem->GameUserInfo.bUserState != USER_WATCH_GAME)
					uselist.push_back(pFindUserItem);
			}
		}
	}
}