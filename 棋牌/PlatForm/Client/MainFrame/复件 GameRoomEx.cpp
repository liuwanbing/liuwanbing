				...
				if ((pDeskStation->bVirtualDesk[i/8]&(1<<(i%8)))!=0)    //change by yjj 090325
				{
					...
					for(int k=0;k<m_pRightWnd->m_UserList.GetItemCount();k++)
					{
						...
						if(pUserInfo->GameUserInfo.bUserState!=USER_PLAY_GAME && pUserInfo->GameUserInfo.dwUserID<0)
						{
							...
							OutputTraceStr("DClient:UserState[%d] n bUserState = USER_PLAY_GAME",pUserInfo->GameUserInfo.dwUserID);
	...
	case ASS_GR_USER_AGREE:		//用户同意
		{
			...
			if (pUserItem!=NULL)
			{
				if (pAgreeGame->bAgreeGame==TRUE) 
				{
					...
					OutputTraceStr("DClient:UserState[%d] n bUserState = USER_ARGEE",pUserItem->GameUserInfo.dwUserID);
				}
				else
				{
					pUserItem->GameUserInfo.bUserState=USER_SITTING;
					OutputTraceStr("DClient:UserState[%d] n bUserState = USER_SITTING",pUserItem->GameUserInfo.dwUserID);
				}
			...
			if ((m_GameInfo.pMeUserInfo->GameUserInfo.bDeskNO!=255)&&(m_GameInfo.pMeUserInfo->GameUserInfo.bDeskNO==pAgreeGame->bDeskNO))
			{
				...
				OutputTraceStr("DClient:UserState[%d] 5 bUserState = USER_ARGEE",m_GameInfo.pMeUserInfo->GameUserInfo.dwUserID);
	...
	case ASS_GR_GAME_BEGIN:		//游戏开始
		{
			...
			for (UINT i=0;i<m_GameInfo.uDeskPeople;i++)
			{
				...
				if (pUserItem!=NULL) 
				{
					...
					OutputTraceStr("DClient:UserState[%d] o bUserState = USER_PLAY_GAME",pUserItem->GameUserInfo.dwUserID);
					m_pUserList->UpdateGameUser(pUserItem);
					//if(m_pFriendList!=NULL)
					//	m_pFriendList->UpdateGameUser(pUserItem);
				}
			...
			if ((m_GameInfo.pMeUserInfo->GameUserInfo.bDeskNO!=255)&&(m_GameInfo.pMeUserInfo->GameUserInfo.bDeskNO==bDeskIndex))
			{
				...
				OutputTraceStr("DClient:UserState[%d] 6 bUserState = USER_PLAY_GAME",m_GameInfo.pMeUserInfo->GameUserInfo.dwUserID);
	...
	case ASS_GR_GAME_END:		//游戏结束
		{
			...
			for (UINT i=0;i<m_GameInfo.uDeskPeople;i++)
			{
				...
				if (pUserItem!=NULL) 
				{
					pUserItem->GameUserInfo.bUserState=USER_SITTING;//游戏刚结束后是坐着的状态
					OutputTraceStr("DClient:UserState[%d] p bUserState = USER_SITTING",pUserItem->GameUserInfo.dwUserID);
					m_pUserList->UpdateGameUser(pUserItem);
					//if(m_pFriendList!=NULL)
					//	m_pFriendList->UpdateGameUser(pUserItem);
			...
			if ((m_GameInfo.pMeUserInfo->GameUserInfo.bDeskNO!=255)&&(m_GameInfo.pMeUserInfo->GameUserInfo.bDeskNO==bDeskIndex))
			{
				...
				OutputTraceStr("DClient:UserState[%d] 7 bUserState = USER_SITTING",m_GameInfo.pMeUserInfo->GameUserInfo.dwUserID);
...
bool CGameRoomEx::OnUserActionQueueSitDown(NetMessageHead * pNetHead, void * pNetData, UINT uDataSize, CTCPClientSocket * pClientSocket)
{
	...
	for (int i = 1; i < m_GameInfo.uDeskPeople + 1; i++)
	{
		...
		if (pUserItem != NULL)
		{
			...
			OutputTraceStr("DClient:UserState[%d] 8 bUserState = USER_SITTING",pUserItem->GameUserInfo.dwUserID);
...
bool CGameRoomEx::OnUserActionSitDown(NetMessageHead * pNetHead, void * pNetData, UINT uDataSize, CTCPClientSocket * pClientSocket)
{
	...
	if (pUserItem!=NULL)
	{
		...
		pUserItem->GameUserInfo.bUserState=pUserSit->bUserState;
		OutputTraceStr("DClient:UserState[%d] q bUserState = %d",pUserItem->GameUserInfo.dwUserID,pUserSit->bUserState);
...
bool CGameRoomEx::OnUserActionStandUp(NetMessageHead * pNetHead, void * pNetData, UINT uDataSize, CTCPClientSocket * pClientSocket)
{
	...
	if (pUserItem!=NULL)
	{
		...
		OutputTraceStr("DClient:UserState[%d] r bUserState = %d",pUserItem->GameUserInfo.dwUserID,pUserSit->bUserState);
...
bool CGameRoomEx::OnUserActionOffLine(NetMessageHead * pNetHead, void * pNetData, UINT uDataSize, CTCPClientSocket * pClientSocket)
{
	...
	if (pUserItem!=NULL)
	{
		...
		OutputTraceStr("DClient:UserState[%d] s bUserState = USER_CUT_GAME",pUserItem->GameUserInfo.dwUserID);
...
bool CGameRoomEx::OnUserActionHitBegin(NetMessageHead * pNetHead, void * pNetData, UINT uDataSize, CTCPClientSocket * pClientSocket)
{
	...
	OutputTraceStr("DClient:UserState[%d] t bUserState = USER_DESK_AGREE",pUserItem->GameUserInfo.dwUserID);
	...
	{//20081212 Fred Huang
		...
		for(int i=0;i<m_GameInfo.uDeskPeople;i++)
		{
			...
			if(pUserInfo)
			{
				...
				OutputTraceStr("DClient:UserState[%d] u bUserState = USER_SITTING",pUserInfo->GameUserInfo.dwUserID);
				int w=rand()%3-1;
				if(w==1)
				{
	...
	{//20081212 Fred Huang
		...
		for(int i=0;i<m_GameInfo.uDeskPeople;i++)
		{
			...
			if(pUserInfo)
			{
				...
				OutputTraceStr("DClient:UserState[%d] v bUserState = USER_ARGEE",pUserInfo->GameUserInfo.dwUserID);
				//pUserInfo->GameUserInfo.bDeskNO=255;
				//pUserInfo->GameUserInfo.bDeskStation=255;
				m_pRightWnd->m_UserList.UpdateGameUser(pUserInfo);
			}
	...
	{//20081212 Fred Huang
		...
		for(int i=0;i<m_GameInfo.uDeskPeople;i++)
		{
			...
			if(pUserInfo)
			{
				//m_DeskFrame.SetUserItem(pNetHead->bAssistantID,i,NULL);
				pUserInfo->GameUserInfo.bUserState=USER_PLAY_GAME;
				OutputTraceStr("DClient:UserState[%d] w bUserState = USER_PLAY_GAME",pUserInfo->GameUserInfo.dwUserID);
				//pUserInfo->GameUserInfo.bDeskNO=255;
				//pUserInfo->GameUserInfo.bDeskStation=255;
				m_pRightWnd->m_UserList.UpdateGameUser(pUserInfo);
			}
...
bool CGameRoomEx::OnAloneDesk(NetMessageHead * pNetHead, void * pNetData, UINT uDataSize, CTCPClientSocket * pClientSocket)
{
	...
	if(1 == pNetHead->bHandleCode)//封
	{
		...
		for(int i=0;i<m_pRightWnd->m_UserList.GetItemCount();i++)
		{
			...
			{
				pUserInfo->GameUserInfo.bUserState=USER_PLAY_GAME;
				OutputTraceStr("DClient:UserState[%d] x bUserState = USER_PLAY_GAME",pUserInfo->GameUserInfo.dwUserID);
	...
	else
	{
		...
		for(int i=0;i<m_GameInfo.uDeskPeople;i++)
		{
			...
			if(pUserInfo)
			{
				...
				OutputTraceStr("DClient:UserState[%d] y bUserState = USER_LOOK_STATE",pUserInfo->GameUserInfo.dwUserID);
				pUserInfo->GameUserInfo.bDeskNO=255;
				pUserInfo->GameUserInfo.bDeskStation=255;
//				m_pRightWnd->m_UserList.UpdateGameUser(pUserInfo);
			}
