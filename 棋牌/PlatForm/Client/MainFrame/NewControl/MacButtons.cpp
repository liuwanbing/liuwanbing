...
//
{
	...
	CBitmap img;
	//已删除图片
	//img.LoadBitmap(IDB_NAV_BACK);///////DANXUAN 单选框背景
	CDC TempDC;
	//TempDC.SetBkMode(TRANSPARENT);//透明的, 显然的, 明晰的背景颜色
	TempDC.CreateCompatibleDC(pDC);
	TempDC.SelectObject(&img);
	dcMem.BitBlt(rectItem.left, rectItem.top, 15,rectItem.Height(),&TempDC,0,0,SRCCOPY);
	dcMem.BitBlt(rectItem.left+15, rectItem.top,15,rectItem.Height(),&TempDC,0,0,SRCCOPY);
	dcMem.BitBlt(rectItem.left+30, rectItem.top,15,rectItem.Height(),&TempDC,0,0,SRCCOPY);
