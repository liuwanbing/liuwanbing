...
int CGameUserManage::GetDeskUserCount(int iDeskIndex)
{
	int n = 0;
	for (INT_PTR i = 0; i < GetOnLineUserCount(); i++)
	{
		if (FindOnLineUserByIndex(i)->GameUserInfo.bDeskNO == iDeskIndex) n++;
	}
	return n;
}

int CGameUserManage::GetDeskPlayCount(int iDeskIndex)
{
	int n = 0;
	for (INT_PTR i = 0; i < GetOnLineUserCount(); i++)
	{
		if (FindOnLineUserByIndex(i)->GameUserInfo.bDeskNO == iDeskIndex && 
			(FindOnLineUserByIndex(i)->GameUserInfo.bUserState == USER_PLAY_GAME || FindOnLineUserByIndex(i)->GameUserInfo.bUserState == USER_CUT_GAME)) 
			n++;
	}
	return n;
}

int CGameUserManage::GetIndexByUserID(int dwUserID)
{
	//查找用户指针
	INT_PTR uActiveCount=m_ActiveUserPtrArray.GetCount();
	for (int i=0;i<uActiveCount;i++)
...
UserItemStruct*  CGameUserManage::FindOnLineUser(BYTE bDeskIndex, BYTE bDeskStation)
{
	//查找用户指针
	INT_PTR uActiveCount=m_ActiveUserPtrArray.GetCount();
	for (int i=0;i<uActiveCount;i++)
	{
		UserItemStruct * pFindUserItem=(UserItemStruct *)m_ActiveUserPtrArray.GetAt(i);
		if ((pFindUserItem!=NULL)&&(pFindUserItem->GameUserInfo.bDeskNO == bDeskIndex && pFindUserItem->GameUserInfo.bDeskStation == bDeskStation)) 
			return pFindUserItem;
	}
	return NULL;
}
