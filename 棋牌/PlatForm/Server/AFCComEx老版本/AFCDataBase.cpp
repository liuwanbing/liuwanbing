...
bool CAFCDataBaseManage::Start()
{
	...
	{
		hDBNative=sqlAddConnection("BZGameLocal.BCF");
	...
	{
		hDBLogon=sqlAddConnection("BZGameLogon.BCF");
	...
	int x=0;
	//连接本地数据库
//	AfxMessageBox(m_strNativeDataConnect);
//	if ((m_pKernelInfo->bNativeDataBase==TRUE)&&(m_DataBaseNative.Open(m_strNativeDataConnect)==false)) 
//		throw new CAFCException(TEXT("CAFCDataBaseManage::Start 连接本地数据库失败"),0x404);

	//连接登陆数据库
	//AfxMessageBox(m_strLogonDataConnect);
//	if ((m_pKernelInfo->bLogonDataBase==TRUE)&&(m_DataBaseLogon.Open(m_strLogonDataConnect)==false))
//		throw new CAFCException(TEXT("CAFCDataBaseManage::Start 连接登陆数据库失败"),0x405);
...
void CAFCDataBaseManage::SQLConnect()
{
	...
	{
		//MessageBox(NULL,L"连接数据库失败！",L"警告",MB_OK|MB_ICONHAND);
		//AfxMessageBox(L"abbbb");
