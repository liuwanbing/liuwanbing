...
bool CBaseMainManage::Init(ManageInfoStruct * pInitData, IDataBaseHandleService * pDataHandleService)
{
	...
	if (!PreInitParameter(&m_InitData,&m_KernelData)) throw new CAFCException(TEXT("CBaseMainManage::Init PreInitParameter 参数调节错误"),0x41A);
	...
	//调用接口
	if (OnInit(&m_InitData,&m_KernelData)==false)  throw new CAFCException(TEXT("CBaseMainManage::Init OnInit 函数错误"),0x41B);
...
unsigned __stdcall CBaseMainManage::LineDataHandleThread(LPVOID pThreadData)
{
	...
	while (1)
	{
		...
		//处理数据
		while(m_pDataLine->GetDataCount())//2008.3.18借用黄工代码改写[问题]当队列中处理信息过多,会不会占用太多的cpu时间而使其他线程延时严重
		{
			//处理数据
			try
			{
