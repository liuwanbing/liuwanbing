...
BOOL CtestDlg::OnInitDialog()
{
	...
	m_talkFileter.LoadFilterMessage(s.GetBuffer());
...
void CtestDlg::OnBnClickedButton1()
{
	...
	const CHAR* tal = m_talkFileter.GetReplaceContent();
	MessageBox(tal);
	wsprintf(szWord,"狗操");
	bool bRet = m_talkFileter.IsAllowableTalk(szWord,strlen(szWord));
	if (bRet)
	{
		AfxMessageBox(szWord);
	}
	wsprintf(szWord,"日");
	bRet = m_talkFileter.IsAllowableTalk(szWord,strlen(szWord));
	if (bRet)
	{
		AfxMessageBox(szWord);
	}
