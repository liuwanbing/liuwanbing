...
STATUS CSoMJFrameLogic::Initialize(ISoGameFactory *pGameFactory, int nGameID, int nTableID)
{
	...
	TCHAR szText[250];
	wsprintf(szText, "log\\MJLog_%s_%d_%d.log", tmp, nGameID, nTableID);
	CLog::Init(szText);
...
int CSoMJFrameLogic::OnPostGameEnd(int nGameEndType)
{
	...
	GetSoGameProcess()->KillTimer(MJ_TIMER_DRAW);
	GetSoGameProcess()->KillTimer(MJ_TIMER_SET_GIVE);
...
*  @sa
*/
int CSoMJFrameLogic::OnGameUserForceQuit(BYTE byChair)
{
	...
	if ( GetSoGameProcess()->GetGameState() == gsPlaying )
	{
		// 逃跑处理
		OnPlayerEscapeInGame(byChair);
...
{
    // 发送游戏配置信息
    OnCollectBaseMsg(uPlayerID, false);
...
{
	_DOUT2("旁观=> OnChangeLookOnSetting(%d, %d)", nPlayerID, bLookOn);

	if ( GetSoGameProcess()->GetGameState() != gsPlaying )
	{
		return 0;  // 非游戏中不旁观玩家进行游戏数据发放
	}
    ...
    if (m_pSoUserMgr != NULL)
    {
        ...
        if (pSoUser != NULL)
        {
			...
			_DOUT2("玩家位置:%d, 设置旁观:%d", nChair, bLookOn);
            ...
            for (i = 0; i < m_TilesSet[nChair].CurrentLength(); i++)
            {
                ...
                {
					// 允许旁观
					if (bLookOn || GetMJSetting().nAnGangShow == 1)
					{
						msg.ucQuadrupletConcealedTiles[msg.ucQuadrupletConcealedNbr] = tileSet.m_ucTile[0];
					}
            ...
            if (m_pSoGameProcessor != NULL)
            {
				_DOUT1("SendGameDataToLookOnUsers发送(%d)位置的旁观数据", nChair);
...
*  例如房间设置项等
*/
int CSoMJFrameLogic::OnCollectBaseMsg(UINT uPlayerID, bool bWatchUser)
{
...
*/
int CSoMJFrameLogic::OnCollectMsg(UINT uPlayerID, bool bWatchUser)
{
    TCMD_NOTIFY_VIEW_INFO cmd;

    // 收集配置及游戏信息等使用，如果重载消息，依然可以通过此函数来填充
    GetViewInfoMsg( uPlayerID, bWatchUser, &cmd);
...
* @param [in] byChair 玩家位置
*/
void CSoMJFrameLogic::OnPlayerEscapeInGame(BYTE byChair)
{
    ...
    ISoUser* pUser = pUserMgr->GetPlayerByChair( byChair );
    if ( NULL == pUser )
    {
        return;
    }
	...
	for (int i = 0; i < PLAYER_NBR; i++ )
	{
		ISoPlayer* pPlayer = pUserMgr->GetPlayerByChair( i );
		if ( pPlayer != NULL && i != nEscapeChair )
		{
			if( cmd.nScore[i] != 0 )
			{
				_snprintf( strEscapeMsg, 127, "玩家 %s 逃跑！扣除金币：%d，您获得了金币：%d。", strName, cmd.nScore[nEscapeChair], nShareScore );
			}
			else
			{
				_snprintf( strEscapeMsg, 127, "玩家 %s 逃跑！扣除金币：%d", strName, cmd.nScore[nEscapeChair] );		
			}

			// 通知客户端
			ShowDlgMsg(strEscapeMsg, sizeof(strEscapeMsg), i, 0, 0);
		}
	}
...
{
    return GetMJSetting().nBaseScore * 40;
}
...
{
    int nScore = abs(nEscapeScore) / PLAYER_NBR;
...
{
    int nMoney = abs(nEscapeScore) / PLAYER_NBR;
