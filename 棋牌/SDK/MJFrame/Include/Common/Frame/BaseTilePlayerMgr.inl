...
int CSoTilePlayerMgr<nPlayerNbr>::CheckDrawBlock( int nChair )
{
	...
	{
		m_pStateEvent->SetMJState( MJ_STATE_GIVE_WAIT );
	}
...
int CSoTilePlayerMgr<nPlayerNbr>::SwitchBlockQuadrupletConcealed( int nNextChair, BYTE ucRequestTile, TCMD_NOTIFY_BLOCK_RESULT &cmd )
{
    ...
    {
		if (ACTION_QUADRUPLET_DNXB == cmd.ucFlag)
		{
			// 东南西北杠显示给所有玩家看
		...
		else if (ACTION_QUADRUPLET_ZFB == cmd.ucFlag)
		{
			// 中发白X杠显示给所有玩家看
		...
		else
		{
			...
			else if (i == nNextChair)
			{
				ISoPlayer * pSoPlayer = GetSoUserManager()->GetPlayerByChair(nNextChair);
				if (pSoPlayer != NULL)
				{
					// 发送给游戏玩家
					GetSoGameProcess()->SendGameDataTo(pSoPlayer->GetUserID(), (char*)&cmd, sizeof(cmd));

					// 检测是否允许旁观
					if (!pSoPlayer->GetBCanBeLookedOn())
					{
						memset(cmd.ucTile, TILE_BEGIN, sizeof(cmd.ucTile));
					}

					// 发送给旁观玩家
					GetSoGameProcess()->SendGameDataToLookOnUsers(nNextChair, (char*)&cmd, sizeof(cmd));
				}
			}
			else
			{
...
void CSoTilePlayerMgr<nPlayerNbr>::UpdateHandTiles( int nChair )
{
	...
	{
		pProcess->SendGameDataToLookOnUsers(nChair, (char*)&cmd, sizeof(cmd));
	}
}
/************************************************************************/
/* 
...
template<int nPlayerNbr>
void CSoTilePlayerMgr<nPlayerNbr>::ShowDlgMsg(char* pMsg, int nLen, BYTE byChair, int nMsgID, BYTE byFlag)
{
    if (NULL == pMsg || nLen < 1)
    {
        return;
    ...
    {
        pProcess->SendGameDataToSeat(byChair, (char *)&cmd, sizeof(cmd));        
		pProcess->SendGameDataToLookOnUsers(byChair, (char *)&cmd, sizeof(cmd));     
    }    
}
