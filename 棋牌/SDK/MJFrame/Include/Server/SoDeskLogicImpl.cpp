	...
	case ASS_GM_FORCE_QUIT:
		{
			...
			{
				// 通知玩家强退事件
				m_pUserNanager->FireEvent(&ISoUserManagerEvent::OnGameUserForceQuit, bDeskStation);
			}
			else
			{
				// 不进行理强退消息处理
	...
	case ASS_SUB_SET_WATCH:        // 旁观设置消息
		{
			...
			UserSetWatch(bDeskStation, (BYTE *)pData, uSize);
		}
		break;
	default:
		{
...
///旁观参数变化
bool CDeskLogicImpl::OnWatchSetChange(BYTE bDeskStation, long int dwTargetUserID, bool bEnableWatch)
{
	//m_SoPlayer[bDeskStation].SetBCanBeLookedOn(bEnableWatch);

	//_DOUT2("玩家位置: %d, 设置旁观状态", bDeskStation, m_SoPlayer[bDeskStation].GetBCanBeLookedOn());

	//// 通知玩家设置旁观事件
	//m_pUserNanager->FireEvent(&ISoUserManagerEvent::OnChangeLookOnSetting, m_SoPlayer[bDeskStation].GetUserID(), (BOOL)bEnableWatch);

	return __super::OnWatchSetChange(bDeskStation, dwTargetUserID, bEnableWatch);
}

///旁观用户坐到游戏桌
BYTE CDeskLogicImpl::WatchUserSitDesk(MSG_GR_S_UserSit * pUserSit, CGameUserInfo * pUserInfo)
{
	return __super::WatchUserSitDesk(pUserSit, pUserInfo);
}
...
BYTE CDeskLogicImpl::UserSitDesk(MSG_GR_S_UserSit* pUserSit, CGameUserInfo* pUserInfo)
{
	if (pUserSit != NULL && pUserInfo != NULL) 
	{
		...
		// 设置是否可以旁观
		m_SoPlayer[bDeskStation].SetBCanBeLookedOn(IsEnableWatch(bDeskStation));

		_DOUT1("玩家位置: %d, 允许旁观", m_SoPlayer[bDeskStation].GetBCanBeLookedOn());
...
bool CDeskLogicImpl::UserAgreeGame(BYTE bDeskStation) 
{
	...
	for (int i = 0; i < PLAY_COUNT; i ++) 
	{
		if (m_pUserInfo[i] != NULL) 
		{
			...
			SendWatchData(i, (void *)&msg, sizeof(msg), MDM_GM_GAME_NOTIFY, ASS_GM_AGREE_GAME, 0);
		}
	}
...
bool CDeskLogicImpl::OnGetGameStation(BYTE bDeskStation, UINT uSocketID, bool bWatchUser)
{
	...
	// 必须先发状态消息2， 后面的180消息才能发送
	// 游戏客户端平台中没有旁观标志, 在此发送旁观变量通知客户端是否为旁观
	if (!bWatchUser)
	{
		SendGameData(bDeskStation, &bWatchUser, sizeof(bWatchUser), MDM_GM_GAME_FRAME, ASS_GM_GAME_STATION, 0);
	}
	else
	{
		SendWatchDataByID(uSocketID, &bWatchUser, sizeof(bWatchUser), MDM_GM_GAME_FRAME, ASS_GM_GAME_STATION, 0);
	}

	// 通知收集游戏基础数据 例如房间设置项等
	m_pGameProcessor->FireEvent(&ISoGameProcessBaseMsgEvent::OnCollectBaseMsg, (UINT)m_pUserInfo[bDeskStation]->m_UserData.dwUserID, bWatchUser);
	
	// 通知收集旁观和短线玩的数据
	m_pGameProcessor->FireEvent(&ISoGameProcessBaseMsgEvent::OnCollectMsg, (UINT)m_pUserInfo[bDeskStation]->m_UserData.dwUserID, bWatchUser);
...
*  @brief 玩家设置旁观状态
*/
int CDeskLogicImpl::UserSetWatch(int nDeskStation, BYTE* pBuffer, int nLen)
{
	MSG_GR_S_SetWatch *pCmd = (MSG_GR_S_SetWatch *)pBuffer;
	if (pCmd == NULL) 
	{
		return -1;
	}
	...
	// 通知客户端消息
	SendGameData(nDeskStation, (void *)pCmd, sizeof(MSG_GR_S_SetWatch), MDM_GM_GAME_NOTIFY, ASS_SUB_SET_WATCH, 0);
	SendWatchData(nDeskStation, (void *)pCmd, sizeof(MSG_GR_S_SetWatch), MDM_GM_GAME_NOTIFY, ASS_SUB_SET_WATCH, 0);

	BOOL bIsEnableWatch = (BOOL)pCmd->bEnableWatch;

	m_SoPlayer[nDeskStation].SetBCanBeLookedOn(bIsEnableWatch);

	_DOUT2("玩家位置: %d, 设置旁观状态", nDeskStation, m_SoPlayer[nDeskStation].GetBCanBeLookedOn());

	// 通知玩家设置旁观事件
	m_pUserNanager->FireEvent(&ISoUserManagerEvent::OnChangeLookOnSetting, m_SoPlayer[nDeskStation].GetUserID(), bIsEnableWatch);
}
