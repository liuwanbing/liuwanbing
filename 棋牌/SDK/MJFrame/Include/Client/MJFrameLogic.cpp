...
void CMJFrameLogic::InitData()
{
	...
	SAFE_CTRL_OPERATE(IButton, CTN_BTN_TuoGuan, SetEnable(false));
	SAFE_CTRL_OPERATE(IButton, CTN_BTN_Cancel_TuoGuan, SetEnable(false));
...
int CMJFrameLogic::HandleGameMessage(WORD nMainID, WORD nSubId, BYTE * buffer, int nLen)
{
	...
	case ASS_GM_GAME_STATION:
		{
			...
			{
				// 旁观标志
				m_bIsWatch = *((bool *)buffer);
			}
		}
		break;
	case ASS_GM_AGREE_GAME:
	//case ASS_SUB_USERAGREE:
		{
	...
	case ASS_SUB_TRUSTEE:
		...
		break;
	case ASS_SUB_SET_WATCH:
		{
			// 旁观设置消息
			MSG_GR_S_SetWatch * pCmd = (MSG_GR_S_SetWatch *)buffer;
			if (pCmd == NULL)
			{
				return 0;
			}
			...
			// 设置旁观状态
			m_bIsEnableWatch = pCmd->bEnableWatch;
		}
		break;
	default:
		{
			...
			if (buffer != NULL && nLen >= sizeof(TCMD_HEADER))
			{
				...
				if (MJFRAME_MOUDLE_MSG == pData->cCmdID)
				{
					...
					case SC_MJ_NOTIFY_GMCMD:         // 接收到GM命令执行结果消息
						...
						break;
					case SC_MJ_NOTIFY_SHOWDLG:       // 接收提示信息
						{
							OnNotifyShowDlg(buffer, nLen);
						}
						break;
					default:
						break;
...
int CMJFrameLogic::OnUIMessage(TUIMessage* pMessage)
{
	...
	else if (pMessage->_uMessageType == UI_MJCTRL_MOVEONMJ)
	{
		if (pMessage->_uControlID == CTN_MJCtrl_HandCtrl_2)
		{
			...
			if (pMessage->_mouseX > 0 && pMessage->_mouseX < m_TilesHand[m_byMyStation].CurrentLength())
			{
				// 播放鼠标移动麻将子上的音效
				PlaySound(SOUND_SEL);
			}
...
int CMJFrameLogic::GameUserLeft(int nIndex)
{
	...
	if (nIndex == m_byMyStation && GetIsWatching())
	{
		// 游戏玩家退出时, 旁观此坐位的玩家也需要跟随退出
		m_pGameFrame->CloseClient();
	}
	...
	else
	{
		...
		if (m_enDiceState == En_Dice_4)
		{
			...
			if (m_stDrawInfo.ucFlag == ACTION_EMPTY)
			{
				...
				if (m_byTurnStation == m_byMyStation && (m_bTuoGuan[m_byMyStation] || m_bTing[m_byMyStation]))
				{
					// 自动处理
					OnRequestGive(TILE_BEGIN, true);
				}
 ...
 * @param[in] bShow   是否显示
 */
void CMJFrameLogic::DrawTing(BYTE byChair, bool bShow)
{
	SAFE_CTRL_OPERATE(IImage, CTN_IMG_Ting_0 + m_pGameFrame->DeskStation2View(byChair), SetControlVisible(bShow));
}
	...
	if (pMJCtrl != NULL)
	{
		...
		for (BYTE i = 0; i < mjSet.CurrentLength(); i++)
		{
			...
			case ACTION_COLLECT:               // 吃
				{
					pMJCtrl->AddOneBlock(LT_COLLECT, tileSet.m_ucTile, m_pGameFrame->DeskStation2View(tileSet.m_ucChair));
				}
				break;
			case ACTION_TRIPLET:               // 碰
				{
					pMJCtrl->AddOneBlock(LT_TRIPLET, tileSet.m_ucTile, m_pGameFrame->DeskStation2View(tileSet.m_ucChair));
				}
				break;
			case ACTION_QUADRUPLET_PATCH:      // 补杠
			case ACTION_QUADRUPLET_REVEALED:   // 明杠
				{
					pMJCtrl->AddOneBlock(LT_QUADRUPLET_REVEALED, tileSet.m_ucTile, m_pGameFrame->DeskStation2View(tileSet.m_ucChair));
				}
				break;
			case ACTION_QUADRUPLET_CONCEALED:  // 暗杠
				{
					...
					pMJCtrl->AddOneBlock(LT_QUADRUPLET_CONCEALED, tileSet.m_ucTile, m_pGameFrame->DeskStation2View(tileSet.m_ucChair));
				}
				break;
			case ACTION_QUADRUPLET_DNXB:       // 东南西北
			case ACTION_QUADRUPLET_ZFB:        // 中发白X
				{
					pMJCtrl->AddOneBlock(LT_QUANDUPLET_SPECIAL, tileSet.m_ucTile, m_pGameFrame->DeskStation2View(tileSet.m_ucChair));
				}
				break;
			default:
				break;
...
void CMJFrameLogic::DrawFinish(TCMD_NOTIFY_FINISH * pCmd, bool bShow)
{
	...
	{
		// 旁观禁用继续按钮
		SAFE_CTRL_OPERATE(IButton, CTN_JSK_BTN_Continue, SetEnable(false));
	}
 ...
 */
void CMJFrameLogic::SetIsWatching(bool bIsWatch)
{
	m_bIsWatch = bIsWatch;
}
 ...
 */
void CMJFrameLogic::SetIsEnableWatch(bool bIsEnableWatch)
{

	MSG_GR_S_SetWatch cmd;
	cmd.bEnableWatch = bIsEnableWatch;
	m_pGameFrame->SendGameData(ASS_SUB_SET_WATCH, (void*)&cmd, sizeof(MSG_GR_S_SetWatch));
}
...
void CMJFrameLogic::OnNotifyGameInfo(BYTE * buffer, int nLen)
{
	...
	m_bIsEnableWatch = pCmd->ucBLookOn;
	...
	if (!GetIsWatching())
	{
		SAFE_CTRL_OPERATE(IButton, CTN_BTN_TuoGuan, SetEnable(true));
		SAFE_CTRL_OPERATE(IButton, CTN_BTN_Cancel_TuoGuan, SetEnable(true));
	}
	...
	for (i = 0; i < PLAYER_NBR; i++)
	{
		...
		m_bTing[i] = pCmd->ucTingType[i] > 0;
		DrawTing(i, m_bTing[i]);
	}
...
void CMJFrameLogic::OnNotifyDeal(BYTE * buffer, int nLen)
{
	...
	for (BYTE i = 0; i < PLAYER_NBR; i++)
	{
		...
		if (i == m_byMyStation)
		{
			if (GetIsWatching() && !GetIsEnableWatch())
			{
				// 旁观玩家且玩家不允许旁观, 只能显示牌背
				m_TilesHand[i].AddTile(TILE_BEGIN, m_tagSetting.nTilesHandNbr + (i == m_byDealer ? 1 : 0));
			}
			else
			{
...
void CMJFrameLogic::OnNotifyBlockResult(BYTE * buffer, int nLen)
{
	...
	case ACTION_TING: // 听
		{
			if (m_bTing[pCmd->ucChair])
			{
				break;
			}

			m_bTing[pCmd->ucChair] = true;
			DrawTing(pCmd->ucChair, m_bTing[pCmd->ucChair]);
		}
		break;
	case ACTION_WIN:  // 胡
		{
	...
	// 设置出牌时间
	if (pCmd->ucChair == m_byMyStation && m_bTing[m_byMyStation])
	{
		SetGameTimer(m_byTurnStation, 1);
	}
	else
	{
		SetGameTimer(m_byTurnStation, m_tagSetting.nTimerGive);
...
{
	TCMD_NOTIFY_CHANGE_LOOKON * pCmd = (TCMD_NOTIFY_CHANGE_LOOKON *)buffer;
	if (pCmd == NULL)
	{
		return ;
	}

	m_TilesHand[m_byMyStation].ReleaseAll();
	for (BYTE i = 0; i < pCmd->ucHandTilesLength; i++)
	{
		m_TilesHand[m_byMyStation].AddTile(pCmd->ucHandTiles[i]);
	}

	m_TilesHand[m_byMyStation].Sort();

	DrawHand(m_byMyStation, m_TilesHand[m_byMyStation], m_TilesHand[m_byMyStation].CurrentLength() % 2 == 0, false);

	int nAnGameCount = 0;
	TILE_SET tileSet;
	for (BYTE i = 0; i < m_TilesSet[m_byMyStation].CurrentLength(); i++)
	{
		m_TilesSet[m_byMyStation].GetSet(i, tileSet);
		if (tileSet.m_ucFlag == ACTION_QUADRUPLET_CONCEALED)
		{
			// 重新设置暗杠牌
			if (nAnGameCount < pCmd->ucQuadrupletConcealedNbr)
			{
				for (BYTE j = 0; j < 4; j++)
				{
					tileSet.m_ucTile[j] = pCmd->ucQuadrupletConcealedTiles[nAnGameCount];
				}

				m_TilesSet[m_byMyStation].SetSet(i, tileSet);
			}
	...
	DrawSet(m_byMyStation, m_TilesSet[m_byMyStation]);
}
 ...
 * @brief 通知显示消息对话框
 */
void CMJFrameLogic::OnNotifyShowDlg(BYTE * buffer, int nLen)
{
	TCMD_MJ_NOTIFY_SHOWDLG * pCmd = (TCMD_MJ_NOTIFY_SHOWDLG *)buffer;
	if (pCmd == NULL)
	{
		return ;
	}

	m_pGameFrame->InsertNormalMessage(pCmd->szMsg);
	AFCMessageBox(pCmd->szMsg, "游戏提示", AFC_CONFIRM);
}
///////////////////////////////////////////////////////////////////////////////
...
void CMJFrameLogic::OnBlockMoveSelect(UINT32 uControlID, int nIndex)
{
	...
	else if (m_byStateSelect == MJ_STATE_SELECT_TING)
	{
		...
		{
			if (tile == m_stBlockInfo.tingGive.GetTile(i))
			{
				for (k = 0; k < m_stBlockInfo.tingWin[i].CurrentLength(); k++)
				{
					tileUserInfo[k].ucTiles[0] = m_stBlockInfo.tingWin[i].GetTile(k);
				}

				pMJCtrl->SetTilesInfo(tileUserInfo, m_stBlockInfo.tingWin[i].CurrentLength());
				pMJCtrl->SetControlVisible(true);
				SAFE_CTRL_OPERATE(IContain, CTN_Block_Panel, SetControlVisible(true));
				return ;
			}
