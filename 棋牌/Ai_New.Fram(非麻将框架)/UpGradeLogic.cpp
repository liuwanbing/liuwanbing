...
bool CUpGradeGameLogic::IsRedTenAndBlackTenAndJ(BYTE byCardList[], int iCardCount)			
{
	if(0x04 != iCardCount)
		return false;
	const BYTE byCardValue[] = {0x29,0x39,0x0A,0x1A,0x2A,0x3A};		//红10,黑10,方J，梅J，红J,黑J
	bool bFlage = true;
	for(int i = 0x00;i < 0x04;i ++)
	{
		bFlage = true;
		for(int j = 0x00;j < 0x06;j ++)
		{
			if(byCardList[i] == byCardValue[j])
			{
				bFlage = false;
...
bool CUpGradeGameLogic::IsRedSixAndBlackSixAndBigKing(BYTE byCardList[], int iCardCount)			
{
	if(0x04 != iCardCount)
		return false;
	const BYTE byCardValue[] = {0x25,0x35,0x4F};
	bool bFlage = true;
	for(int i = 0x00;i < 0x04;i ++)
	{
		bFlage = true;
