...
const int CARD_COUNT_PER_PERSON = 45;		// 每玩家手中牌数
...
{
	void operator= (const OutCardStruct &out)
	{
		iCardCount = out.iCardCount;
		memcpy(iCardList,out.iCardList,sizeof(BYTE)*CARD_COUNT_PER_PERSON);
	}
	int					iCardCount;						//扑克数目
...
struct peiOutCardStruct
{
	void operator =(peiOutCardStruct p)
	{
		out.iCardCount = p.out.iCardCount;
		memcpy(out.iCardList,p.out.iCardList,sizeof(BYTE)*out.iCardCount);
		type = p.type;
	}
	OutCardStruct out;
	///代表配好的牌的种类，0为单牌，1为鬼，2为2的，3为四个的炸弹，4为三顺，5为双顺，6为单顺，7三张，8对子，9为三张带一张，10为三张带一对
	///，11为三顺带单，12为三顺带对子,13为四带两单，14为四带两对，15代表不出
	int type;                   
};
