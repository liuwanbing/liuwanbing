...
#define ID_TAKE_CARD                106                 //等待闲家出牌
...
public:
	...
	BYTE					m_byOpenCardCount[PLAY_COUNT];					//已经亮牌数量
...
public:
 ...
 public:
	...
	//下注按钮
	void	OnHandleHitBetBt(int iHitButton);
	//叫庄按钮
	void	OnHandleHitNtBt();
	//放弃叫庄按钮
	void	OnHandleHitGiveUpNtBt();
	//要牌/停牌按钮
	void	OnHandleHitTakeCardBt(int iHitButton);
	...
	//------------------------------------------------
	//处理服务端发过来的游戏要牌结果消息
	void	OnHandleTakeCardResult(void *buffer);
	//处理服务端发过来的游戏停牌消息
	void	OnHandleStopCard(void *buffer);
	//处理服务端停牌结果消息
	void	OnHandleStopPaiResult(void *buffer);
	//处理玩家退出消息
...
//--------------------------------------------------------------------------------
	...
	//设置数字控件是否要显示符号位
	void	SetINumSingle(const int iNumID,__int64 iNum,bool bFlag);
