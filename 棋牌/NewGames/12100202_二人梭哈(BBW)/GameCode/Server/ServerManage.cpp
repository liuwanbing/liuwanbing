...
BOOL CServerGameDesk::LoadIni()
{
	...
	key.Format("cardshape");
	m_dwCardShape &=0x00000000;
	m_dwCardShape |= ((f.GetKeyVal(key,"MinStraight",0)<<1)&0xFFFFFFFF);//最小顺子
	return true;
}
...
BOOL CServerGameDesk::LoadExtIni(int iRoomID)
{
	...
	m_iBaseScale = f.GetKeyVal(key, "BaseScale",m_iBaseScale);
	...
	m_byAddNum= f.GetKeyVal(key, "AddNum", m_byAddNum);///**< 每轮加注次数 0时不做限制*/   //lym 2011-12-13;										
	m_bPK= f.GetKeyVal(key,"CanPK",0);
	int iFollow = f.GetKeyVal(key, "userfollow",m_byUserFollow);
	if (iFollow < 0)
	{
		m_byUserFollow = 0;
	}
	...
	case TIME_WAIT:
		{
			...
			CString cst;
			if (NULL!=m_pUserInfo[m_iOutCardPeople] && NULL != m_pUserInfo[m_iNowBigPeople] && \
				m_bUserState[m_iOutCardPeople] && m_bUserState[m_iNowBigPeople])
			{
				cst.Format("wlog::当前下注的玩家%s，最大玩家%s",m_pUserInfo[m_iOutCardPeople]->m_UserData.nickName,m_pUserInfo[m_iNowBigPeople]->m_UserData.nickName);
			}
			else
			{
				cst.Format("wlog::m_iOutCardPeople==%d,m_iNowBigPeople===%d",m_iOutCardPeople,m_iNowBigPeople);
			}
...
BOOL CServerGameDesk::SetLimitNote()
{
	...
	//if(m_iBaseNote <= 0)
	{
	    m_iBaseNote= GetRoomLimitBasePoint();
	}	
...
bool CServerGameDesk::GameBegin(BYTE bBeginFlag)
{
	...
	CString str;
	str.Format("xyh_postion:m_iAIHaveWinMoney=%I64d,m_iReSetAIHaveWinMoney=%I64d,m_bAIWinAndLostAutoCtrl=%d",m_iAIHaveWinMoney,m_iReSetAIHaveWinMoney,m_bAIWinAndLostAutoCtrl);
...
BOOL CServerGameDesk::SendCard()
{
	...
	{
		m_iUserCard[iSendCardPos%PLAY_COUNT][iSendCardPos/PLAY_COUNT] =  m_iTotalCard[((iSendCardPos%PLAY_COUNT) * 5)+(iSendCardPos%5)];
	}
...
BOOL CServerGameDesk::BeginPlay()
{
	...
	m_iUpGradePeople = GetWeight(iPoint, iCount);	 // 因为都无法看到牌，所有随机让某一个玩家先下注
	...
	case TYPE_ADD://加注数=当前最大注数-用户已经下注数+加注数
		{
			...
			{
				iNoteVal = m_iUserNote[m_iNowBigPeople]+1;	// 本轮最大押注数=最大押注数+加注数
			}
