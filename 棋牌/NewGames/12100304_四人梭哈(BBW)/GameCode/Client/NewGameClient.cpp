...
{
	/*CString sdp;
	sdp.Format("lwlog::游戏消息 客户端 消息[ID=%d]", nSubId);
	OutputDebugString(sdp);*/
...
#else
	sprintf(pchar, "%s/resources/agree.bmp", CBcfFile::GetAppPath());
#endif
...
#else
	sprintf(pchar, "%s/resources/pass.bmp", CBcfFile::GetAppPath());
#endif
...
{
	int notepoint = m_iNoteTotalPoint;
	m_MyUI.SetTextInfo(CTN_1000000_CTN_100210728_TEXT_100210745, notepoint, bFlag, AlignmentCenter);
	return;
}
...
{
	int iView = ViewStation(bDeskStation);
	int iPerJuTotalNote = m_iPerJuTotalNote[bDeskStation];
	m_MyUI.SetTextInfo(CTN_1000000_CTN_100210728_TEXT_100210781 + iView, iPerJuTotalNote, bFlag, AlignmentCenter);
