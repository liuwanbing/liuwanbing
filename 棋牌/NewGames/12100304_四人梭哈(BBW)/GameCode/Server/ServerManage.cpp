...
BOOL CServerGameDesk::BeginPlay()
{
	...
	m_iUpGradePeople = GetFirstGreaterCard();//GetWeight(iPoint, iCount);	 // 因为都无法看到牌，所有随机让某一个玩家先下注
...
int CServerGameDesk::GetFirstGreaterCard(void)//第一轮谁先出牌调用函数
{
	int iGreatest = -1;
	for(int i = 0; i < PLAY_COUNT; i++)
	{
		...
		int iFirNum = m_Logic.GetCardNum(m_iUserCard[iGreatest][1]);
		int iSecNum = m_Logic.GetCardNum(m_iUserCard[i][1]);
		if(iFirNum == iSecNum)
		{
			int iFirHua = m_Logic.GetCardHuaKind(m_iUserCard[iGreatest][1]);
			int iSecHua = m_Logic.GetCardHuaKind(m_iUserCard[i][1]);
