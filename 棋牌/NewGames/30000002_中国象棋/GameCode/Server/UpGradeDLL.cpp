...
extern "C" __declspec(dllexport) BOOL GetServiceInfo(ServerDllInfoStruct * pServiceInfo, UINT uVer)
{
	...
	//写入信息
	TCHAR strServerDLL[MAX_NAME_INFO];//JMod-SERVER_DLL_NAME-20090213
	GET_SERVER_DLL_NAME(strServerDLL);
	...
	lstrcpy(pServiceInfo->szDLLFileName,strServerDLL);
	lstrcpy(pServiceInfo->szWriter,TEXT("龙浩天"));
	lstrcpy(pServiceInfo->szDllNote,TEXT("象棋游戏组件"));
