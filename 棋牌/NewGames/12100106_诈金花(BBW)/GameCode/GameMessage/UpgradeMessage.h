...
/*-------------------------------------------------------------------------------*/
//换牌数据包
struct	ChangeCardData
{
	BYTE		byDeskStation;		//换牌玩家座位
	BYTE		byCard;				//换牌的数据
	ChangeCardData()
	{
		memset(this,255,sizeof(ChangeCardData));
	}
};
//换牌结果数据包
struct	ChangeCardResultData
{
	BYTE		byDeskStation;		//换牌玩家座位
	BYTE		byUserCard[MAX_CARD_COUNT];		//换牌玩家的牌数据
	ChangeCardResultData()
	{
		memset(this,255,sizeof(ChangeCardResultData));
	}
};
/*-------------------------------------------------------------------------------*/
...
#define ASS_SUPER_RESULT                121             //超端控制结果消息
#define ASS_SUPER_USER                  122             //超端验证消息
