...
CUpGradeGameLogic::CUpGradeGameLogic(void)
{
	...
	m_iReplaceCardArray[0] = 0x4E;	//小鬼
	m_iReplaceCardArray[1] = 0x4F;	//大鬼

	m_iReplaceCardCount = 2;		//代替牌队例长度
}

///设置代替出牌队列
///@param BYTE iCardList[] [in] 牌队列9
///@param int iCardCount [in]   牌的张数
///@return 函数返回值为TRUE
...
///获取队例中可代替牌张数
///@param BYTE iCardList[] [in]扑克队列
///@param int iCardCount   [in] 扑克的张数
///@return 函数返回提取的扑克张数
...
///获取指定牌个数
///@param  BYTE iCardList[] [in] 要处理的数组
///@param  int iCardCount	[in] 要处理的牌的数量
///@param  BYTE bCard   [in] 指定的牌值
///@return 返回该牌值的数量
...
bool CUpGradeGameLogic::IsVariationShape(BYTE  iCardList[],int iCardCount,BYTE iUpCard[],int iUpCount)
{
	...
	{
		if(CountPoint(byCard,iCount)%10!=0)	//没有作为替代牌
		{
			return true;
		}
	}
								   ...
								   BYTE iSecondCard[], int iSecondCount,BYTE iSecondUpCard[])
{
	...
	else
	{
		...
		if(bFirstShape != bSecondShape)	//变种牌型是同牌型中最小的牌
		{
