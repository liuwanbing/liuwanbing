								   ...
								   bool bFirstOut)
{
	...
	if( iBaseCardShape < iOutCardShape)						//牌形>
	{
	//if(iOutCount!=iBaseCount)//牌型不一致
	//{
	//	if(UG_SLAVE_510K <= iOutCardShape)					//炸弹
	//	{
	//		if(iOutCardShape>iBaseCardShape)
	//		{
	//			return true;
	//		}
	//	}	
...
void CUpGradeGameLogic::Set510KPoint(BYTE bBaseCardList[],int iBaseCount,BYTE b510kList[],int i510KCount)
{
	...
	{
		cs.Format("test::GetCardNum(bCardList[%d])==%d",i,GetCardNum(bCardList[i]));
		OutputDebugString(cs);
	}
	cs.Format("test::iPoint==%d",iPoint);
	OutputDebugString(cs);*/
}
//计算得分(510k)
